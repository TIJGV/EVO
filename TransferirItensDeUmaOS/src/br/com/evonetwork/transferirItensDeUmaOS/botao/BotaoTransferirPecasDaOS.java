...
public class BotaoTransferirPecasDaOS implements AcaoRotinaJava {
	
	// botão na tabela TGFITE

	@Override
	public void doAction(ContextoAcao ca) throws Exception {
		System.out.println("***EVO - TRANSFERINDO PEÇAS OS - INICIO***");
		...
		for (Registro linha : linhas) {
			try {
				System.out.println("Percorrendo peças");
		...
		System.out.println("***EVO - TRANSFERINDO PEÇAS OS - FIM***");
	}
		...
		try {
			...
			if (rset.next()) {
				if(!"A".equals(rset.getString("STATUS")) && rset.getString("STATUS") != null)
					throw new Exception("A Ordem de Serviço precisa estar aberta para devolver peças!");
			} else {
		...
		try {
			...
			if (rset.next()) {
				if(!"L".equals(rset.getString("STATUSNOTA")))
					throw new Exception("A nota precisa estar confirmada para devolver peças!");
				
				if(rset.getBigDecimal("CODTIPOPER").compareTo(BigDecimal.valueOf(700)) != 0)
					throw new Exception("Tipo de operação inválido para devolução de peças!");
			}
		} catch (Exception e) {
			e.printStackTrace();
			throw new Exception(e.getMessage());
