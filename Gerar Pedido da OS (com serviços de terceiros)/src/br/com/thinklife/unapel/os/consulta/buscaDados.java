...
public class buscaDados
{
    ...
    public static String consultaTCSPRO(final BigDecimal numos) {
        final String sql = "select \r\n"
        		...
        		+ "TIPOMOVIMENTO,\r\n"
        		+ "(SELECT DISTINCT ITE2.CODLOCALORIG \r\n"
        		+ "FROM TGFITE ITE, TGFVAR VAR, TGFITE ITE2\r\n"
        		+ "WHERE VAR.NUNOTA = (SELECT MAX(VAR1.NUNOTA) FROM TGFVAR VAR1 WHERE ITE.NUNOTA = VAR1.NUNOTAORIG AND ITE.SEQUENCIA = VAR1.SEQUENCIAORIG AND VAR1.STATUSNOTA = 'L')\r\n"
        		+ "AND ITE2.NUNOTA = VAR.NUNOTA AND (ITE2.SEQUENCIA*-1) = VAR.SEQUENCIA\r\n"
        		+ "AND ITE.NUNOTA = TCS.NUMREQUISICAO AND ITE.SEQUENCIA = TCS.SEQREQUISICAO) AS LOCAL_DEST,\r\n"
        		+ "coalesce(LOTE,'null') as LOTE\r\n"
        		+ "from AD_TCSPRO TCS\r\n"
        		+ "where NVL(NUNOTAPED,0) = 0 AND STATUS IN ('E','DP') and NUMOS = "+numos+"\r\n"
        		+ "order by TIPOMOVIMENTO asc";
        return sql;
    }
    ...
    public static String consultaSQL(final BigDecimal numos) {
        final String sql = "select \n"
        		...
        		+ "TOTHORASAPONTDEC\n"
        		+ "FROM AD_TCSITE ITE\n"
        		+ "WHERE ITE.NUMOS = A.NUMOS AND ITE.NUMITEM = A.NUMITEM),2)) AS APONTAMENTOS,\n"
        		+ "a.CODUSURESP\n"
        		+ "from ad_tcsite a\n"
        		+ "where A.numos= "+numos+"\n"
        		+ "and A.AD_NROPEDVENDA is null \n"
        		+ "AND (SELECT AD_GERCOB FROM TSIUSU USU WHERE a.CODUSURESP = USU.CODUSU) = 'S'  \n"
